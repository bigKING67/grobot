...
impl ToolExecutor for LocalToolExecutor {
    fn execute_tool_call(
        ...
        if tool_name == TOOL_MCP_SERVERS {
            validate_projected_tool_args_visible(
                &context,
                args,
                TOOL_MCP_SERVERS,
                "validate_mcp_servers_args_visible",
                "Use ready_only in normal mcp_servers surfaces; full_debug is required to include disabled MCP servers.",
            )?;
        }
