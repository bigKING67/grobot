...
- 当 provider 为 Kimi 且模型命中 `k2.5`（并开启 `provider_options.kimi.files_enabled`）时，`read` 会对 `PDF(未传 pages)/image/video` 自动切换到 Kimi 文件+多模态链路；`meta.extra` 会附带 `remote_provider/remote_model/remote_file_id` 等可观测字段。
