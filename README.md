...
- 控制层（`.grobot/`）：
  ...
  - `.grobot/config.toml.example`：cc-connect 运行配置模板
  - `.grobot/config.toml`：本地实例配置（含敏感信息，已 gitignore）
  - `.grobot/rules/`、`.grobot/skills/`、`.grobot/sessions/`：规则、技能、会话产物

### 配置初始化

```bash
cp .grobot/config.toml.example .grobot/config.toml
```
...
```bash
# 一次性问答
/Users/gaoqian/Documents/sixseven/workman/groland/grobot/grobot start \
  --project grobot \
  --work-dir "$(pwd)" \
  --message "请介绍当前目录要做什么"
...
```bash
# 只检查本地配置解析（不访问模型服务）
/Users/gaoqian/Documents/sixseven/workman/groland/grobot/grobot status \
  --project grobot \
  --work-dir "$(pwd)"
...
/Users/gaoqian/Documents/sixseven/workman/groland/grobot/grobot serve \
  --project grobot \
  --work-dir "$(pwd)"
