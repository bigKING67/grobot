...
function resolveStartupTerminalColumns(): number | undefined {
...
export function printRunStartBanner(input: RunStartBannerInput): void {
  ...
  const uiRenderer = createCliUiRenderer({
    ...
    stdoutIsTTY: Boolean((process.stdout as unknown as { isTTY?: boolean }).isTTY),
    terminalColumns: resolveStartupTerminalColumns(),
  });
