...
import { persistRunStartModelToConfig } from "../../cli/start/model-config-sync";

async function main(): Promise<void> {
  const tempRoot = `${process.cwd()}/.tmp-start-model-config-sync-${Date.now()}-${Math.floor(Math.random() * 10_000)}`;
  ...
  try {
    ...
    const payload = {
      ...
      missing_config_path_failed:
        ...
        && missingPathResult.message.includes("config_toml path unavailable"),
      empty_model_failed:
        !emptyModelResult.ok
        && emptyModelResult.message.includes("target model is empty"),
      missing_file_failed:
        !missingFileResult.ok
        && missingFileResult.message.includes("cannot read config_toml"),
    };
