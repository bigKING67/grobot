...
async function main(): Promise<void> {
  ...
  const payload = {
    mcp_strict_failure_is_human_surface:
      ...
      mcpPlain.includes(
        "原因: missing_required_MCP_rule_packs_for_servers:_grok-search",
      ),
    ...
    scheduler_tick_error_is_human_surface:
      ...
      schedulerTickPlain.includes(
        "原因: Error:_failed_to_parse_schedule_window",
      ),
    ...
    runtime_interrupt_ignored_is_human_surface:
      ...
      runtimeInterruptIgnoredPlain.includes(
        "/interrupt 请求发出时，当前回合已完成或已过安全中断点。",
      ),
    rewind_capture_failed_is_human_surface:
      stripAnsi(rewindCaptureFailedSurface).includes("● 检查点保存失败") &&
      stripAnsi(rewindCaptureFailedSurface).includes(
        "本轮对话已继续，但这一步无法用于 /rewind 回退。",
      ) &&
      stripAnsi(rewindCaptureFailedSurface).includes(
        "原因: Error:_checkpoint_store_unavailable",
      ),
