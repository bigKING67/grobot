import {
  extractTypeScriptAstDependencyTargets,
  isTypeScriptDependencyAstRuntimeAvailable,
} from "../../tools/context/graph/dependency-ts-ast";
...
function runCli(argv: string[]): number {
  ...
  switch (command) {
    case "extract": {
      ...
      const targets = extractTypeScriptAstDependencyTargets(filePath, content);
      process.stdout.write(`${JSON.stringify({
        ast_runtime_available: isTypeScriptDependencyAstRuntimeAvailable(),
        targets,
      })}\n`);
      return 0;
    }
...
const shouldRun = entryScript.includes("dependency-ast-contract");
if (shouldRun) {
  try {
    process.exitCode = runCli(process.argv.slice(2));
  } catch (error) {
    process.stderr.write(`dependency-ast-contract fatal: ${String(error)}\n`);
    process.exitCode = 1;
  }
}
