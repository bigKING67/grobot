...
async function main(): Promise<void> {
  ...
  const payload = {
    ...
    pending_root_ask_only: pendingAskTopLevel.filter((item) => item.command === "/ask").length >= 1
      && pendingAskTopLevel.every((item) => item.command === "/ask" || !item.command.startsWith("/ask")),
    model_filter_only_model_related: modelOnly.every((item) => item.command.startsWith("/model")),
    ask_filter_only_ask_related: askOnly.every((item) => item.command.startsWith("/ask")),
    ask_filter_single_command: askOnly.length === 1 && askOnly[0]?.command === "/ask",
