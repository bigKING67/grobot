export type OptionValue = string | boolean;

export interface ParsedArgs {
  command: string;
  options: Record<string, OptionValue>;
  positionals: string[];
}

export function usage(): string {
  return [
    "Grobot TS dev CLI (source-checkout fallback)",
    "No subcommand => enter local interactive CLI (bare `grobot`).",
    "",
    "Commands:",
    "  (default)            Local interactive CLI (stdin TTY required for full UX)",
    "  status [--project <name>] [--work-dir <dir>] [--home <dir>] [--project-root <dir>] [--config <path>] [--provider <name>] [--api-key <key>] [--base-url <url>] [--model <id>] [--probe] [--json] [--context-graph-cache-window-size <n>] [--context-graph-cache-degrade-hit-rate <0..1>] [--context-graph-cache-degrade-min-entries <n>] [--context-persistent-graph-degrade-parsed-rate <0..1>] [--context-persistent-graph-degrade-reused-rate <0..1>] [--context-persistent-graph-degrade-removed-rate <0..1>] [--context-persistent-graph-degrade-min-entries <n>] [--context-persistent-graph-degrade-min-scanned-files <n>] [--gateway-impl ts] [--runtime-impl rust] [--shadow-mode|--no-shadow-mode]",
    "  init --global [--home <dir>] | --project [--project-root <dir>] [--hooks-samples]",
    "  gc [--scope global|project|all] [--retention-days <n>] [--keep-recent-sessions <n>] [--keep-recent-plans-per-session <n>] [--dry-run|--apply] [--json]",
    "  start [--message <text>] [--project <name>] [--work-dir <dir>] [--home <dir>] [--project-root <dir>] [--config <path>] [--provider <name>] [--session-scope dm|group] [--session-subject <id>] [--history-turns <n>] [--handoff-recent-turns <n>] [--handoff-auto-on-exit|--no-handoff-auto-on-exit] [--resume [<session-id|query>]] [--resume-last] [--resume-all] [--fork-session] [--rewind [<checkpoint-id|query>]] [--rewind-mode both|conversation|code|summarize] [--resume-session-at <checkpoint-id>] [--rewind-files <path1,path2>] [--circuit-failures <n>] [--circuit-cooldown-secs <n>] [--provider-max-inflight <n>] [--provider-requests-per-minute <n>] [--provider-burst <n>] [--session-backend auto|file|redis] [--redis-url <url>] [--require-redis true|false] [--allow-redis-fallback true|false] [--gateway-impl ts] [--runtime-impl rust] [--verbose] [--trace]",
    "  serve [--project <name>] [--work-dir <dir>] [--home <dir>] [--project-root <dir>] [--config <path>] [--bind 127.0.0.1:8080] [--management-token <token>] [--config-read-policy auto|public|auth|disabled] [--session-backend auto|file|redis] [--redis-url <url>] [--require-redis true|false] [--allow-redis-fallback true|false] [--gateway-impl ts] [--runtime-impl rust]",
    "",
    "Probe notes:",
    "  --probe uses base_url/api_key from CLI flags first, then GROBOT_BASE_URL/GROBOT_API_KEY.",
    "  --json emits a machine-readable status snapshot.",
    "  --verbose for `start` keeps full diagnostic event logs; default interactive mode uses compact diagnostics.",
    "  --trace extends --verbose with interaction trace lifecycle lines.",
    "  Redis strict mode defaults to enabled when backend=redis. Use --allow-redis-fallback true or GROBOT_ALLOW_REDIS_FALLBACK=1 for temporary local fallback.",
    "  --context-graph-cache-window-size controls recent-turn graph cache window size (default 20, env GROBOT_CONTEXT_GRAPH_CACHE_WINDOW_SIZE).",
    "  --context-graph-cache-degrade-hit-rate sets degradation threshold (default 0.30, env GROBOT_CONTEXT_GRAPH_CACHE_DEGRADE_HIT_RATE).",
    "  --context-graph-cache-degrade-min-entries sets minimum window entries before degradation check (default 8, env GROBOT_CONTEXT_GRAPH_CACHE_DEGRADE_MIN_ENTRIES).",
    "  --context-persistent-graph-degrade-parsed-rate sets parsed/scanned max threshold (default 0.35, env GROBOT_CONTEXT_PERSISTENT_GRAPH_DEGRADE_PARSED_RATE).",
    "  --context-persistent-graph-degrade-reused-rate sets reused/scanned min threshold (default 0.55, env GROBOT_CONTEXT_PERSISTENT_GRAPH_DEGRADE_REUSED_RATE).",
    "  --context-persistent-graph-degrade-removed-rate sets removed/scanned max threshold (default 0.20, env GROBOT_CONTEXT_PERSISTENT_GRAPH_DEGRADE_REMOVED_RATE).",
    "  --context-persistent-graph-degrade-min-entries sets minimum persistent window entries (default 8, env GROBOT_CONTEXT_PERSISTENT_GRAPH_DEGRADE_MIN_ENTRIES).",
    "  --context-persistent-graph-degrade-min-scanned-files sets minimum scanned files for persistent degradation checks (default 40, env GROBOT_CONTEXT_PERSISTENT_GRAPH_DEGRADE_MIN_SCANNED_FILES).",
    "  `start` is IM-only. Without explicit platform/session context it exits with code 2 and suggests bare `grobot`.",
    "  `gc` defaults to dry-run. pass `--apply` to actually delete aged runtime artifacts.",
    "  `gc` policy source: CLI args > [storage.cleanup] in config.toml > built-in defaults.",
    "",
    "Optional session args for start:",
    "  --platform feishu|telegram --tenant <id> --scope dm|group --subject <id> (legacy aliases; prefer --session-scope/--session-subject)",
    "  --resume <session-id|query> supports slash-like matching at startup (exact id > query matcher > latest resumable fallback).",
    "  --resume-last keeps explicit latest-resumable semantics; --resume-all widens query scope to include active session.",
    "  --rewind [checkpoint-id|query] supports startup checkpoint matching (exact id > query matcher > latest checkpoint fallback).",
    "  --rewind-mode both|conversation|code|summarize controls startup rewind behavior; defaults to code when --rewind-files is set, else both.",
    "  --resume-session-at <checkpoint-id> remains a legacy rewind alias and now participates in startup selector flow.",
  ].join("\n");
}

export function parseArgs(argv: string[]): ParsedArgs {
  const positionals: string[] = [];
  const options: Record<string, OptionValue> = {};
  let index = 0;
  while (index < argv.length) {
    const token = argv[index];
    if (!token.startsWith("--")) {
      positionals.push(token);
      index += 1;
      continue;
    }

    const eqIndex = token.indexOf("=");
    if (eqIndex >= 0) {
      const key = token.slice(2, eqIndex);
      const value = token.slice(eqIndex + 1);
      options[key] = value;
      index += 1;
      continue;
    }

    const key = token.slice(2);
    const next = argv[index + 1];
    if (typeof next === "string" && !next.startsWith("--")) {
      options[key] = next;
      index += 2;
      continue;
    }
    options[key] = true;
    index += 1;
  }

  const command = positionals[0] ?? "";
  return {
    command,
    options,
    positionals: positionals.slice(1),
  };
}

export function readOptionString(options: Record<string, OptionValue>, key: string): string | undefined {
  const value = options[key];
  if (typeof value === "string") {
    const trimmed = value.trim();
    return trimmed.length > 0 ? trimmed : undefined;
  }
  return undefined;
}

export function readOptionStringAny(options: Record<string, OptionValue>, keys: readonly string[]): string | undefined {
  for (const key of keys) {
    const value = readOptionString(options, key);
    if (value) {
      return value;
    }
  }
  return undefined;
}

export function hasFlag(options: Record<string, OptionValue>, key: string): boolean {
  const value = options[key];
  if (typeof value === "boolean") {
    return value;
  }
  if (typeof value === "string") {
    const normalized = value.trim().toLowerCase();
    if (normalized === "false" || normalized === "off" || normalized === "0" || normalized === "no") {
      return false;
    }
    return normalized.length > 0;
  }
  return false;
}

function isTruthyString(value: string | undefined): boolean {
  if (!value) {
    return false;
  }
  const normalized = value.trim().toLowerCase();
  return (
    normalized === "true" ||
    normalized === "1" ||
    normalized === "yes" ||
    normalized === "on"
  );
}

export function validateHardCutExecutionOptions(options: Record<string, OptionValue>): string[] {
  const errors: string[] = [];
  if (hasFlag(options, "legacy-python-cli")) {
    errors.push("--legacy-python-cli is removed in TS+Rust hard-cut mode");
  }
  if (isTruthyString(process.env.GROBOT_LEGACY_PYTHON)) {
    errors.push("GROBOT_LEGACY_PYTHON is no longer supported");
  }

  const gatewayRaw = readOptionString(options, "gateway-impl");
  if (gatewayRaw) {
    const gatewayValue = gatewayRaw.trim().toLowerCase();
    if (gatewayValue === "python") {
      errors.push("--gateway-impl=python is no longer supported");
    } else if (gatewayValue !== "ts") {
      errors.push(`invalid --gateway-impl value: ${gatewayRaw}`);
    }
  }

  const runtimeRaw = readOptionString(options, "runtime-impl");
  if (runtimeRaw) {
    const runtimeValue = runtimeRaw.trim().toLowerCase();
    if (runtimeValue === "python") {
      errors.push("--runtime-impl=python is no longer supported");
    } else if (runtimeValue !== "rust") {
      errors.push(`invalid --runtime-impl value: ${runtimeRaw}`);
    }
  }

  return errors;
}
