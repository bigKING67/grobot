...
type ContextRetrievalConfig = {
  ...
  embedding: RetrievalRemoteConfig | null;
  rerank: RetrievalRemoteConfig | null;
  source: string;
  enabled_source: string;
  selected_limit_source: string;
  candidate_limit_source: string;
  shared_base_url: string;
  shared_base_url_source: string;
  shared_api_key_source: string;
  embedding_source: string;
  rerank_source: string;
  embedding_disabled_reason: string | null;
...
function includeAsPass(line: string): string {
...
function includeAsFail(line: string): string {
...
function extractSectionsFromHistory(history: JsonObject[]): Record<string, string[]> {
...
function renderCompactSnapshot(sections: Record<string, string[]>): string {
  const lines: string[] = [HISTORY_COMPACT_HEADER, ""];
  const ordered = [
    SECTION_ARCHITECTURE,
    SECTION_MODIFIED,
    SECTION_VERIFICATION,
...
function findExistingSnapshot(rows: JsonObject[]): string | null {
...
function trimHistoryMessages(history: JsonObject[], maxTurns: number): JsonObject[] {
...
function parseSnapshotSections(snapshot: string): Record<string, string[]> {
  const sections: Record<string, string[]> = {};
  let current = "";
...
function buildRetrievedContextBlock(history: JsonObject[], userPrompt: string, retrievalConfig: ContextRetrievalConfig | null): string {
...
function parseEmbeddingRows(response: JsonObject): Map<number, number[]> {
  ...
  for (const rowRaw of dataRaw) {
    ...
    const vector = embedding.filter((item): item is number => typeof item === "number");
    rows.set(index, vector);
  }
  return rows;
}

function computeEmbeddingSimilarityScores(candidates: JsonObject[], response: JsonObject): Record<number, number> {
...
function computeRerankScores(candidates: JsonObject[], response: JsonObject): Record<number, number> {
  const scores: Record<number, number> = {};
...
function readNested(base: JsonObject, key: string): JsonObject {
  const value = base[key];
  return isObject(value) ? value : {};
}

function resolveContextRetrievalConfig(
  projectToml: JsonObject,
  fallbackApiKey: string | null,
  globalToml: JsonObject
  ...
  if (!enabled) {
    return {
      ...
      selected_limit_source: projectSection.selected_limit !== undefined ? "project" : globalSection.selected_limit !== undefined ? "global" : "default",
      candidate_limit_source: projectSection.candidate_limit !== undefined ? "project" : globalSection.candidate_limit !== undefined ? "global" : "default",
      shared_base_url: "",
  ...
  const selectedLimitSource = projectSection.selected_limit !== undefined ? "project" : globalSection.selected_limit !== undefined ? "global" : envKey ? "env" : "default";
  const candidateLimitSource = projectSection.candidate_limit !== undefined ? "project" : globalSection.candidate_limit !== undefined ? "global" : envKey ? "env" : "default";
  const sharedBaseUrl = String(projectSection.base_url ?? globalSection.base_url ?? envBaseUrl ?? "").trim();
  const sharedBaseUrlSource = projectSection.base_url !== undefined ? "project" : globalSection.base_url !== undefined ? "global" : envBaseUrl ? "env" : "default";
  const sharedApiKey = String(projectSection.api_key ?? globalSection.api_key ?? envKey ?? fallbackApiKey ?? "").trim();
  const sharedApiKeySource = projectSection.api_key !== undefined ? "project" : globalSection.api_key !== undefined ? "global" : envKey ? "env" : fallbackApiKey ? "fallback" : "default";
  ...
  const embeddingSource = projectEmbedding.model !== undefined ? "project" : globalEmbedding.model !== undefined ? "global" : envEmbeddingModel ? "env" : "off";
  ...
  const rerankSource = projectRerank.model !== undefined ? "project" : globalRerank.model !== undefined ? "global" : envRerankModel ? "env" : "off";

  const embedding: RetrievalRemoteConfig | null =
    sharedBaseUrl && sharedApiKey && embeddingModel
      ? {
      ...
      : null;
  const rerank: RetrievalRemoteConfig | null =
    sharedBaseUrl && sharedApiKey && rerankModel
      ? {
          base_url: sharedBaseUrl,
          api_key: sharedApiKey,
          model: rerankModel,
  ...
  switch (command) {
    case "trim": {
      ...
      const history = Array.isArray(historyRaw)
        ? historyRaw.filter((item): item is JsonObject => isObject(item))
        : [];
    ...
    case "retrieved-block": {
      ...
      const retrieval = isObject(payload.retrieval_config) ? (payload.retrieval_config as ContextRetrievalConfig) : null;
    ...
    case "embedding-scores": {
      ...
      const candidates = Array.isArray(candidatesRaw)
        ? candidatesRaw.filter((item): item is JsonObject => isObject(item))
        : [];
