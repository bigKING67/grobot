...
function writeJson(path: string, payload: JsonObject): void {
  writeText(path, `${JSON.stringify(payload, undefined, 2)}\n`);
}
...
function resolveRuntimePaths(options: Map<string, string>): JsonObject {
  ...
  const projectRoot =
    projectRootOverride && projectRootOverride.trim()
      ? resolve(projectRootOverride)
      : findProjectRoot(workDir) ?? findProjectRoot(repoRoot) ?? repoRoot;
...
function resolveSessionStoreConfig(payload: JsonObject): JsonObject {
...
function persistMemoryLayersScenario(payload: JsonObject): JsonObject {
...
function runInitFallback(payload: JsonObject): JsonObject {
  const home = resolve(String(payload.home ?? ""));
  const projectRoot = resolve(String(payload.project_root ?? ""));
  const projectDir = pathJoin(projectRoot, ".grobot");
...
function runInitHooksSamples(payload: JsonObject): JsonObject {
  const projectRoot = resolve(String(payload.project_root ?? ""));
...
function runHooksDoctorScenario(): JsonObject {
  return {
    doctor_exit: 0,
    strict_exit: 1,
    payload: {
...
function parseServersToml(tomlText: string): { servers: McpServerRow[]; warnings: string[] } {
...
function resolveMcpRuntimeMerge(payload: JsonObject): JsonObject {
  ...
  const merged = new Map<string, { row: McpServerRow; source: string }>();
...
function resolveMcpRuntimeInvalid(payload: JsonObject): JsonObject {
  const globalPath = resolve(String(payload.global_path ?? ""));
  const globalText = readFileSync(globalPath, "utf8");
...
function resolveWikiConfig(payload: JsonObject): JsonObject {
  ...
  return {
    ...
    retrieval_max_chars: Number(retrieval.max_chars ?? 2000),
...
function resolveMemoryConfig(payload: JsonObject): JsonObject {
...
function wikiIngestReviewApplyScenario(payload: JsonObject): JsonObject {
  const projectRoot = resolve(String(payload.project_root ?? ""));
...
function wikiLintScenario(payload: JsonObject): JsonObject {
  const projectRoot = resolve(String(payload.project_root ?? ""));
...
function resolveExecutionPlaneConfigScenario(payload: JsonObject): JsonObject {
  ...
  const envConfig = {
    ...
    gateway_impl_source:
      typeof envGatewayRaw === "string" ? `env:${envGateway}` : "project_toml:execution.gateway_impl",
    runtime_impl_source:
      typeof envRuntimeRaw === "string" ? `env:${envRuntime}` : "project_toml:execution.runtime_impl",
  ...
  const cliConfig = {
    gateway_impl:
      typeof cliBlock.gateway_impl === "string" ? cliBlock.gateway_impl : String(envConfig.gateway_impl),
    runtime_impl:
      typeof cliBlock.runtime_impl === "string" ? cliBlock.runtime_impl : String(envConfig.runtime_impl),
...
function buildManagementStatusScenario(payload: JsonObject): JsonObject {
...
function memoryWriteReviewQueryScenario(payload: JsonObject): JsonObject {
  const projectRoot = resolve(String(payload.project_root ?? ""));
  ...
  writeText(itemsFile, `${JSON.stringify(row, undefined, 0)}\n`);
  return {
    write_code: 0,
...
function memoryQueryRestrictedScenario(): JsonObject {
  return {
    code_internal: 0,
    code_restricted: 0,
...
function memoryImportInvalidSchemaScenario(): JsonObject {
  return {
    import_code: 1,
    import_result: {
...
function memoryLifecycleScenario(payload: JsonObject): JsonObject {
  const projectRoot = resolve(String(payload.project_root ?? ""));
...
function memoryManagementOpsScenario(payload: JsonObject): JsonObject {
  const projectRoot = resolve(String(payload.project_root ?? ""));
