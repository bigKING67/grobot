...
async function runGatewayContractSmoke() {
  ...
  const runtimeToolRecoveryTimelineResult = runCommand("npx", [
    "--yes",
    "--package",
    "tsx@4.20.6",
    "tsx",
    "gateway/src/extensions/contracts/runtime-tool-recovery-timeline-contract.ts",
  ]);
  assertSuccess("runtime-tool-recovery-timeline-contract", runtimeToolRecoveryTimelineResult);
  const runtimeToolRecoveryTimelinePayload = parseJsonOutput(
    "runtime-tool-recovery-timeline-contract",
    runtimeToolRecoveryTimelineResult.stdout,
  );
  assert.equal(runtimeToolRecoveryTimelinePayload.ok, true);
  assert.equal(runtimeToolRecoveryTimelinePayload.active_timeline_count, 2);
  assert.equal(typeof runtimeToolRecoveryTimelinePayload.active_latest_recovery_key, "string");
  assert.equal(runtimeToolRecoveryTimelinePayload.active_health_level, "risk");
  assert.equal(runtimeToolRecoveryTimelinePayload.active_health_score, 36);
  ...
  const runtimeToolRecoveryReadinessResult = runCommand("npx", [
    ...
    "tsx@4.20.6",
    "tsx",
    "gateway/src/extensions/contracts/runtime-tool-recovery-readiness-contract.ts",
  ]);
  assertSuccess("runtime-tool-recovery-readiness-contract", runtimeToolRecoveryReadinessResult);
  const runtimeToolRecoveryReadinessPayload = parseJsonOutput(
    "runtime-tool-recovery-readiness-contract",
    runtimeToolRecoveryReadinessResult.stdout,
  );
  assert.equal(runtimeToolRecoveryReadinessPayload.ok, true);
  assert.equal(runtimeToolRecoveryReadinessPayload.ready_status, "pass");
  assert.equal(runtimeToolRecoveryReadinessPayload.ready_passed, true);
  assert.equal(runtimeToolRecoveryReadinessPayload.degraded_status, "warn");
  assert.equal(runtimeToolRecoveryReadinessPayload.degraded_passed, true);
  assert.equal(runtimeToolRecoveryReadinessPayload.degraded_reason, "degraded_auto_recovery_allowed");
  ...
  assert.equal(runStartPlanModeContractPayload.open_plan_surface_hides_old_stored_state_label, true);
  assert.equal(runStartPlanModeContractPayload.open_plan_surface_splits_quality_lines, true);
  ...
  assert.equal(runStartPlanModeContractPayload.script_plan_surface_hides_old_stored_state_label, true);
  assert.equal(runStartPlanModeContractPayload.script_plan_surface_splits_quality_lines, true);
...
async function runTsRustExecutionSmoke() {
  ...
  assert.equal(planModeFlowPayload.plan_status_has_short_next_line, true);
  assert.equal(planModeFlowPayload.plan_status_has_focus_line, true);
  assert.equal(planModeFlowPayload.plan_status_splits_quality_lines, true);
  assert.equal(planModeFlowPayload.plan_status_hides_redundant_stored_state, true);
  assert.equal(planModeFlowPayload.plan_status_avoids_duplicate_focus, true);
  assert.equal(planModeFlowPayload.plan_status_avoids_duplicate_guard, true);
