...
登录态任务默认先走 TMWebDriver（TMWD）：`web_scan` / `web_execute_js` 操作用户真实浏览器，保留当前标签页、Cookie 和登录态。JSReverser-MCP / remote-debugging CDP 只在需要 Network initiator、script source、Debugger、preload hook、AST/VMP 等 DevTools 级能力时进入；若 debug Chrome 是新窗口或独立 profile，需要用户在该 debug Chrome 中单独登录，不能假设它继承真实浏览器登录态。
...
- 操控跨域 iframe
- 文件上传（通过 TMWD bridge / DevTools 能力，例如 `DOM.setFileInputFiles`）

登录态逆向建议顺序：
1. 先用 TMWebDriver 在真实浏览器确认目标请求、页面状态、Cookie/session 与触发动作。
2. 如果只需提取请求样本或可见运行时状态，继续使用 TMWebDriver，不强制切到 remote-debugging CDP。
3. 如果必须看 initiator、脚本源码、断点或 preload hook，再切到 JSReverser-MCP / remote-debugging CDP，并明确这是 `remote_cdp_debug_browser` 上下文；必要时让用户在 debug Chrome 中单独登录。
