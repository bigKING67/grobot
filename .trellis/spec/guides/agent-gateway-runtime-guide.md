...
default check order, no duplicate monolithic smoke execution, complete runner
coverage for every `runtime-tool-*.ts` contract, release describe coverage, CI
trigger/toolchain coverage, and per-process temporary fixture isolation for
runtime-tool contracts. Run the focused suite during runtime-tool iteration.
Runner JSON output must preserve `schema_version: 1`,
...
Status/release code must derive `action_required` and the surface-specific
default `actionable_next_step` from this registry instead of copying
